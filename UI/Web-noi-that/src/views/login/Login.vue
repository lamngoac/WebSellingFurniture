        ...
        <a-form id="components-form-demo-normal-login" :form="form" class="login-form" @submit="handleSubmit">
            ...
            <a-form-item>
              ...
              'Password',
              {
                ...
                <div class="text-error-password" v-if="!checkExistPassword">
            ...
            <a-form-item style="margin-top: 15px">
              ...
              'remmber',
              {
                ...
                initialValue: true,
                    ...
                    <span class="font-bold">Nhớ mật khẩu</span>
                </a-checkbox>
                <div class="btn-login">
                ...
                <div class="font-bold">
                    Hoặc
                    <router-link class="h-router-link" :to="{path:'/register'}">
                        Đăng ký ngay
                    </router-link>
                </div>
            </a-form-item>
...
export default {
    ...
    methods: {
        ...mapActions(ModuleUser, ["getUsers","getUserByUserName"]),
                ...
                if (!err) {
                    ...
                    if (me.users && me.users.length > 0) {
                        ...
                        if (accountUsers && accountUsers.length > 0) {
                            ...
                            if (passUsers && passUsers.length > 0) {
                                if(values.remmber){
                                    // lưu tạm thông tin user lên local 
                                    localStorage.setItem("User",JSON.stringify({Username : values.UserName,Password : values.Password}));
                                }
                                else{
                                    localStorage.removeItem("User");
                                }
                                let res = await me.getUserByUserName(values.UserName)
                                if(accountUsers[0].role && accountUsers[0].role == me.$enum.Role.Admin){
                                    // điều hướng sang admin
                                    me.$router.push("/admin"); 
                                    localStorage.setItem("userName",res.userName);
                                } else {
                                    // điều hướng trang chủ
                                    me.$router.push({ path: "/",name:"home", params:{ id: res.userName ,user:res }})
                                    localStorage.setItem("userName",res.userName);
                                }
                            }
                            // sai mật khẩu
                            else {
                                me.checkExistPassword = false;
